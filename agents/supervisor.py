    ...
    def _synthesize_single_agent_response(self, question: str, raw_result: str, agent_type: str) -> Optional[str]:
        ...
        if agent_type == "data":
            ...
            # Check for responses that start with Genie headers followed by JSON
                ...
                needs_synthesis = True
            # Check for DEBUG INFO sections (indicates raw output that needs cleaning)
            elif 'DEBUG INFO' in raw_result or 'DEBUG: Question:' in raw_result or 'DEBUG: Message ID:' in raw_result:
                needs_synthesis = True
            # Check for raw data arrays or excessive technical details
            elif '["' in raw_result and '"]' in raw_result and raw_result.count('[') > 3:
                needs_synthesis = True
            # Check for "Raw Query Results" markers (indicates unprocessed output)
            elif 'Raw Query Results:' in raw_result or '🤖 SQL Generated by Genie:' in raw_result:
                needs_synthesis = True
